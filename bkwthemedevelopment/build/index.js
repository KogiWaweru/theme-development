...
class Like {
  ...
  createLike(currentLikeBox) {
    jquery__WEBPACK_IMPORTED_MODULE_0___default().ajax({
      beforeSend: xhr => {
        xhr.setRequestHeader('X-WP-None', universityData.nonce);
      },
      ...
      success: response => {
        currentLikeBox.attr('data-esists', 'yes');
...
class MyNotes {
  ...
  createNote(e) {
    ...
    jquery__WEBPACK_IMPORTED_MODULE_0___default().ajax({
      ...
      data: ourNewPost,
      success: response => {
        jquery__WEBPACK_IMPORTED_MODULE_0___default()(".new-note-title , .new-note-body").val(''); //clear the field once user submit their notes

        jquery__WEBPACK_IMPORTED_MODULE_0___default()(`
                    <li data-id="${response.id}">
                    <input readonly class="note-title-field" type="" value="${response.title.raw}">
                    <span class="edit-note"> <i class="fa fas-pencil" aria-hidden="true"></i> Edit</span>
                    <span class="delete-note"> <i class="fa fas-trash-o" aria-hidden="true"></i> Delete</span>
                    <textarea readonly class="note-body-field">${response.content.raw}</textarea>
                    <span class="update-note btn btn--blue btn--small"> <i class="fa fas-arrow-right" aria-hidden="true"></i> Save</span>
                </li>
                    `).prependTo("#my-notes").hide().slideDown(); //outputing the user notes on the web
  ...
  getResult() {
    jquery__WEBPACK_IMPORTED_MODULE_0___default().getJSON(universityData.root_url + '/wp-json/university/v1/search?term=' + this.searchField.val(), results => {
      this.resultDiv.html(`
          ...
          ${results.generalInfo.length ? '<ul class="link-list min-list">' : '<p>No items match your search </p>'}
    ...
    //     this.resultDiv.html(`
    //     <h2 class="search-overlay__section-title">Main content</h2>
    //     ${combinedResults.length ? '<ul class="link-list min-list">' : '<p>No items match your search </p>'}
    //      ${combinedResults.map(item => `<li> <a href ="${item.link}"> ${item.title.rendered} </a> </li>` ).join('')}
    //     ${combinedResults.length ? ' </ul>' : ''}
    //   `);
    //   this.isSpinnerVisible = false;
    // });
  } //  by ${item.type == 'post' ? by `${item.authorName}` : ''}
